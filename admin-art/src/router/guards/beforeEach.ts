...
async function handleRouteGuard(
  ...
  // 3. 处理动态路由注册（登录/注册等认证页不走菜单初始化，避免 next(false) 白屏）
  const isAuthPage =
    to.path === RoutesAlias.Login ||
    to.path === '/auth/register' ||
    to.path === '/auth/forget-password'

  if (!routeRegistry?.isRegistered() && userStore.isLogin && !isAuthPage) {
    // 防止并发请求（快速连续导航场景）
...
async function handleDynamicRoutes(
  ...
  try {
    ...
    // 401 错误：axios 拦截器已处理退出登录；必须导航到登录页，避免 next(false) 白屏
