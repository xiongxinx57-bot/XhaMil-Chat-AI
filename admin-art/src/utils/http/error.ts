 ...
 * @returns 错误对象
 */
export function handleError(error: AxiosError<ErrorResponse & { message?: string }>): never {
  // 处理取消的请求
  ...
  const serverMessage = (body?.message || body?.msg || '').trim()
